...
이 데이터는 8강 전체에서 같은 기준으로 사용한다. CSV는 원본 피드, JSON은 검증 규칙, HTML은 웹 표 구조, TXT는 사람이 읽는 스키마 설명을 담당한다. 학생에게는 먼저 CSV 헤더와 JSON 키를 읽게 한 뒤 문제 풀이를 시작하게 한다.

오류 행은 일부러 포함되어 있다. 빈 제목, 잘못된 가격 문자열, 허용되지 않은 category, 음수 stock, 허용되지 않은 status, 중복 record_id가 각각 어떤 오류 코드로 분류되는지 확인한다.
