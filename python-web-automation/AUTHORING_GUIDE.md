...
학생용 통합 노트북, `lecture.md`, `mission.md`, `final-mission.md` 에는 아래 패턴을 넣지 않는다. 파일 용량 기준을 맞추기 위한 자동 패딩은 실패로 본다.

- `### 체크포인트 1`, `### 체크포인트 2` 처럼 번호만 바뀌는 자동 생성 체크포인트.
- `### 보강 설명 1`, `### 보강 설명 2` 처럼 같은 문장 구조를 반복하는 분량 채우기.
- “이때 학생은 화면에 보이는 값 하나보다 …” 처럼 모든 강의에 복붙되는 일반론.
- 학생에게 실제 행동을 요구하지 않는 추상적 운영 문구.
- 같은 문단을 말만 조금 바꿔 3회 이상 반복하는 filler.

체크포인트가 필요하면 `## 디버깅 체크포인트` 한 절로 묶고, 그 레슨의 실제 데이터, 코드, selector, 저장 파일과 직접 연결된 항목만 쓴다.
...
한도 미달은 자동 보강 지시가 아니라 리뷰 신호다. 같은 문장 구조를 반복하거나 `체크포인트 N`, `보강 설명 N` 같은 번호형 문단으로 채우면 실패다. 보강은 반드시 해당 레슨의 실제 코드, 데이터, selector, 저장 산출물과 연결되어야 한다.

- **lecture.md 가 18KB 미달**: 새 filler를 넣지 말고 HTTP/HTML 개념, selector 실수 사례, 안전 규칙, 현업 사례를 레슨 데이터 기준으로 구체화한다.
- **mission.md 가 14KB 미달**: 문제 수가 15개 미만인지 먼저 확인한다. 각 문제의 입력 데이터, 기대 결과 형태, 빈칸 scaffold, 힌트를 다르게 만든다.
- **solution.md 가 24KB 미달**: 각 정답 아래 `왜 이 코드가 정답인지`, selector 기준, 오답 패턴, 채점 포인트를 문제별로 다르게 보강한다.
- **final mission 이 미달**: 시나리오, 산출물 정의, 평가 루브릭, 제출 전 체크를 실제 제출 흐름과 연결해 늘린다.
- **teacher guide 가 미달**: 2시간 타임라인, 문제별 채점 포인트, 막히는 지점별 멘트를 구체화한다.
- 기준을 맞추기 위해 의미 없는 반복을 넣어야 한다면 용량 기준을 억지로 맞추지 말고 사람 검수로 넘긴다.
...
- [ ] 학생용/웹교안에 `체크포인트 N`, `보강 설명 N`, 동일 문단 반복 같은 자동 패딩 문구가 없음
